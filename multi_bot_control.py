...
def run_bot_with_reconnect(bot, bot_name):
    ...
    while True:
        try:
            ...
            # auto_reconnect=True xử lý các lần mất kết nối tạm thời
            bot.gateway.run(auto_reconnect=True)
        except Exception as e:
            # Bắt các lỗi nghiêm trọng, in thông báo và chờ 5 giây trước khi thử lại
...
def create_bot(token, bot_name, is_main=False, is_main_2=False):
                            ...
                            try:
                                ...
                                for msg_item in messages:
                                    ...
                                    if author_id == karibbit_id and "embeds" in msg_item and len(msg_item["embeds"]) > 0:
                                        ...
                                        return # Thoát sau khi xử lý xong
                            except Exception as e:
    ...
    # --- KHỞI CHẠY BOT TRÊN MỘT LUỒNG RIÊNG VỚI CƠ CHẾ RECONNECT ---
...
# --- Auto Work Functions (Giữ nguyên logic gốc) ---
...
# =================================================================
# === GIAO DIỆN WEB (FLASK) - GIỮ NGUYÊN GIAO DIỆN GỐC ===
# =================================================================
app = Flask(__name__)
...
def index():
    ...
    if request.method == "POST":
        # --- Xử lý gửi tin nhắn thủ công và nhanh (Chỉ dùng acc phụ) ---
        ...
        # --- Xử lý gửi mã theo acc (ĐÃ CẬP NHẬT LOGIC) ---
            ...
            try:
                ...
                if not codes_list:
                    ...
                    target_bot = None
                    # Nếu index là của acc cuối cùng (sly_dd), dùng main_bot
                    if acc_index == len(acc_names) - 1:
                        target_bot = main_bot
                        print(f"[Send Codes] Chọn Acc Chính 1 ({acc_names[acc_index]}) để gửi mã.")
                    # Ngược lại, dùng acc phụ tương ứng
                    else:
                        if acc_index < len(bots):
        ...
        # --- Xử lý Spam (Chỉ dùng acc phụ) ---
            ...
            if spam_enabled and not spam_message:
                msg_status = "Vui lòng nhập nội dung trước khi bật spam."
                spam_enabled = False
            else:
        ...
        # --- Xử lý Auto Work (Chỉ dùng acc phụ) ---
    ...
    # --- Chuẩn bị dữ liệu để render template ---
    ...
    # Cập nhật acc_options để bao gồm tất cả tên trong acc_names
    ...
    for i, name in enumerate(acc_names):
        if i < len(bots): # Chỉ thêm các acc phụ có tồn tại
            reboot_options += f'<option value="sub_{i}">Acc Phụ {i} ({name})</option>'

    return render_template_string(HTML.format(**locals()))
...
def keep_alive():
    """Hàm này chỉ để giữ cho chương trình chính không bị tắt."""
    while True:
        time.sleep(3600) # Ngủ một tiếng
...
if __name__ == "__main__":
    ...
    with bots_lock:
        ...
        for i, token in enumerate(tokens):
            # Lấy tên từ danh sách, nếu thiếu thì dùng tên mặc định
    ...
    # Chạy web server trên luồng chính để dễ quản lý hơn
