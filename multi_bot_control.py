# karuta_deep_complete_final.py - Discord Multi-Bot Control System
import discum
import threading
import time
import os
...
class BotController:
                        ...
                        if max_num < threshold:
                            ...
                            def grab():
                                try:
                                    ...
                                    logging.error(f"[Bot {bot_num}] Grab error: {e}")
    ...
    def start_spam(self):
        ...
        if not self.spam_thread_running:
            ...
            logging.info("[Spam] Started spam functionality")
    ...
    def _spam_loop(self):
                        ...
                        for bot in self.bots:
                            if bot:
                                try:
                                    ...
                                    time.sleep(0.5)  # Short delay between bots
                                except Exception as e:
        ...
        self.spam_thread_running = False
        logging.info("[Spam] Spam loop ended")
    ...
    def start_auto_work(self):
        ...
        if not self.work_thread_running:
            ...
            logging.info("[Auto Work] Started auto work functionality")

    def _auto_work_loop(self):
        """Auto work loop implementation - Complete version"""
        while self.auto_work_enabled and self.work_thread_running:
            try:
                logging.info("[Auto Work] Starting work cycle for all accounts...")
                
                # Run work for each account with proper delay
                for acc_index in range(len(self.tokens)):
                    ...
                    if acc_index < len(self.bots) and self.bots[acc_index]:
                        ...
                        threading.Thread(target=self._run_work_for_account, args=(self.tokens[acc_index], acc_index)).start()
                        time.sleep(10)  # 10 second delay between accounts
                    else:
                        logging.warning(f"[Auto Work] Bot {acc_index} not available")
                        
                if self.auto_work_enabled:
                    logging.info("[Auto Work] Work cycle completed. Waiting 12+ hours for next cycle...")
                    # Wait 12+ hours with random variation
                    wait_time = 12 * 3600 + random.randint(0, 3600)  # 12-13 hours
                    time.sleep(wait_time)
                    
            except Exception as e:
                logging.error(f"[Auto Work] Loop error: {e}")
                time.sleep(60)  # Wait 1 minute before retrying
                
        self.work_thread_running = False
        logging.info("[Auto Work] Auto work loop ended")

    def _run_work_for_account(self, token, acc_index):
        """Run work commands for a specific account - Complete implementation"""
        try:
            # Create dedicated work bot
            work_bot = discum.Client(token=token, log=False)
            
            def send_karuta_command():
                try:
                    work_bot.sendMessage(self.work_channel_id, "kc o:ef")
                    logging.info(f"[Acc {acc_index}] Sent: kc o:ef")
                except Exception as e:
                    logging.error(f"[Acc {acc_index}] Error sending kc o:ef: {e}")

            def send_kn_command():
                try:
                    work_bot.sendMessage(self.work_channel_id, "kn")
                    logging.info(f"[Acc {acc_index}] Sent: kn")
                except Exception as e:
                    logging.error(f"[Acc {acc_index}] Error sending kn: {e}")

            def send_kw_command():
                try:
                    work_bot.sendMessage(self.work_channel_id, "kw")
                    logging.info(f"[Acc {acc_index}] Sent: kw")
                except Exception as e:
                    logging.error(f"[Acc {acc_index}] Error sending kw: {e}")
            ...
            def click_tick(channel_id, message_id, custom_id, application_id, guild_id):
                try:
                    ...
                    logging.info(f"[Acc {acc_index}] Clicked button: {custom_id}")
                except Exception as e:
                    logging.error(f"[Acc {acc_index}] Error clicking button: {e}")
            ...
            def on_message(resp):
                if resp.event.message:
                    ...
                    if author == self.karuta_id and channel == self.work_channel_id:
                        # Handle work responses
                        if "kjw" in content.lower():
                            code_match = re.search(r'kjw (\w+)', content.lower())
                            if code_match:
                                code = code_match.group(1)
                                logging.info(f"[Acc {acc_index}] Found kjw code: {code}")
                                threading.Timer(2, lambda: work_bot.sendMessage(self.work_channel_id, f"kjw {code}")).start()
                        
                        elif "kjn" in content.lower():
                            code_match = re.search(r'kjn (\w+)', content.lower())
                            if code_match:
                                code = code_match.group(1)
                                logging.info(f"[Acc {acc_index}] Found kjn code: {code}")
                                threading.Timer(2, lambda: work_bot.sendMessage(self.work_channel_id, f"kjn {code}")).start()
                        
                        # Handle button interactions
                        if "components" in msg and len(msg["components"]) > 0:
                                ...
                                if "components" in component_row:
                                    for component in component_row["components"]:
                                        if component.get("type") == 2:  # Button
                                            ...
                                            if "✅" in component.get("emoji", {}).get("name", ""):
                                                logging.info(f"[Acc {acc_index}] Found tick button: {custom_id}")
                                                threading.Timer(
                                                    2,
                                                    lambda: click_tick(
                                                        channel, msg["id"], custom_id,
                                                        self.karuta_id, msg.get("guild_id")
                                                    )
                                                ).start()

            # Start work sequence
            work_bot.gateway.run(auto_reconnect=False)
            threading.Timer(1, send_karuta_command).start()
            threading.Timer(5, send_kn_command).start()
            threading.Timer(10, send_kw_command).start()
            
            # Keep work bot running for responses
            time.sleep(30)
            work_bot.gateway.close()
            
        except Exception as e:
            logging.error(f"[Acc {acc_index}] Lỗi work: {str(e)}")
            ...
            for i, code in enumerate(codes_list):
                if code.strip():
                    ...
                    threading.Timer(
                        delay * i, 
                        send_delayed_code, 
                        args=(bot, final_msg, acc_name)
                    ).start()
            
            logging.info(f"[Card Code] Started sending {len(codes_list)} codes via {acc_name}")
            return True
            
        except Exception as e:
            logging.error(f"[Card Code] Error: {str(e)}")
            return False

    def send_card_code(self, acc_name, code):
        """Send single card code - kept for compatibility"""
        try:
            acc_index = self.acc_names.index(acc_name) if acc_name in self.acc_names else 0
            return self.send_card_codes(acc_index, "", [code], 1)
        except Exception as e:
            logging.error(f"[Card Code] Error sending single code: {e}")
            return False
    ...
    <div class="container">
        ...
        <section class="bot-status">
            ...
            <div class="bot-card" id="mainBot1">
                ...
                <div class="bot-controls">
                    <div class="control-group">
                        ...
                        <button class="toggle-btn" id="autoGrab1" onclick="dashboard.toggleAutoGrab('main_1')">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="heartThreshold1" value="50" min="1" 
                               onchange="dashboard.setHeartThreshold('main_1', this.value)">
                    </div>
                    <button class="action-btn" onclick="dashboard.rebootBot('main_1')">
            ...
            <div class="bot-card" id="mainBot2">
                ...
                <div class="bot-controls">
                    <div class="control-group">
                        <label>Auto Grab</label>
                        <button class="toggle-btn" id="autoGrab2" onclick="dashboard.toggleAutoGrab('main_2')">
                            <i class="fas fa-power-off"></i>
                            <span>TẮT</span>
                        </button>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" id="heartThreshold2" value="50" min="1" 
                               onchange="dashboard.setHeartThreshold('main_2', this.value)">
                    </div>
                    <button class="action-btn" onclick="dashboard.rebootBot('main_2')">
                        <i class="fas fa-redo"></i> KHỞI ĐỘNG LẠI
                    </button>
                </div>
            </div>
        ...
        <section class="control-panels">
            ...
            <div class="control-panel">
                ...
                <div class="panel-content">
                    ...
                    <button class="action-btn send-btn" onclick="dashboard.sendMessage()">
                        <i class="fas fa-paper-plane"></i> GỬI QUA TẤT CẢ BOT
                    </button>
                    
                    <div class="quick-commands">
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc o:w')">
                            <i class="fas fa-hammer"></i> kc o:w
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc o:ef')">
                            <i class="fas fa-tools"></i> kc o:ef
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc o:p')">
                            <i class="fas fa-cog"></i> kc o:p
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:1')">
                            <i class="fas fa-gem"></i> kc e:1
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:2')">
                            <i class="fas fa-gem"></i> kc e:2
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:3')">
                            <i class="fas fa-gem"></i> kc e:3
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:4')">
                            <i class="fas fa-gem"></i> kc e:4
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:5')">
                            <i class="fas fa-gem"></i> kc e:5
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:6')">
                            <i class="fas fa-gem"></i> kc e:6
                        </button>
                        <button class="quick-cmd-btn" onclick="dashboard.sendQuickCommand('kc e:7')">
            ...
            <div class="control-panel">
                ...
                <div class="panel-content">
                    <div class="control-group">
                        ...
                        <button class="toggle-btn" id="spamToggle" onclick="dashboard.toggleSpam()">
                            <i class="fas fa-power-off"></i>
                            <span>TẮT</span>
                        </button>
                    </div>
                    <div class="form-group">
                        <label>Tin nhắn spam</label>
                        <textarea id="spamMessage" placeholder="Nhập tin nhắn spam..."></textarea>
                    </div>
                    <div class="form-group">
            ...
            <!-- Auto Work Control Panel -->
            <div class="control-panel">
                <h3><i class="fas fa-hammer"></i> AUTO WORK</h3>
                <div class="panel-content">
                    <div class="control-group">
                        <label>Trạng thái</label>
                        <button class="toggle-btn" id="autoWorkToggle" onclick="dashboard.toggleAutoWork()">
                            <i class="fas fa-power-off"></i>
                            <span>TẮT</span>
                        </button>
                    </div>
                    <p style="color: var(--text-secondary); font-size: 0.9rem; margin-top: 10px;">
                        ...
                        Tự động thực hiện work cho tất cả accounts theo chu kỳ 12 tiếng.
                    </p>
                </div>
            </div>

            <!-- Card Code Control Panel -->
            <div class="control-panel">
                <h3><i class="fas fa-credit-card"></i> CARD CODE</h3>
                <div class="panel-content">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Delay (giây)</label>
                        <input type="number" id="cardDelay" value="11" min="1" step="0.1">
                    </div>
                    <div class="form-group">
                        <label>Prefix (tùy chọn)</label>
                        <input type="text" id="cardPrefix" placeholder="Ví dụ: #">
                    </div>
                    <div class="form-group">
                        <label>Danh sách mã (cách nhau bằng dấu phẩy)</label>
                        <textarea id="cardCodes" placeholder="code1, code2, code3..."></textarea>
                    </div>
                    <button class="action-btn card-btn" onclick="dashboard.sendCardCode()">
        ...
        <div class="terminal">
            <div class="terminal-header">
                ...
                <button class="clear-btn" onclick="dashboard.clearTerminal()">
    ...
    <script>
        class DashboardController {
            ...
            updateBotStatus(data) {
                ...
                // FIX: Button state synchronization from server
                ...
                subBotsStatus.innerHTML = `<i class="fas fa-circle"></i><span>${data.sub_bots.connected}/18 TRỰC TUYẾN</span>`;
                
                // Update sub bot list
                this.updateSubBotList(data.sub_bots);

                // Update feature toggles - FIX: Sync with server state
            ...
            updateSubBotList(subBots) {
                ...
                for (let i = 0; i < 18; i++) {
                    ...
                    subBotItem.innerHTML = `
                        <div class="sub-bot-name">Acc ${i} (${accNames[i]})</div>
                        <div class="sub-bot-status">
                            <i class="fas fa-circle"></i>
                            <span>${i < subBots.connected ? 'TRỰC TUYẾN' : 'NGOẠI TUYẾN'}</span>
                        </div>
                        <button class="sub-bot-reboot" onclick="dashboard.rebootBot('sub_${i}')">
            ...
            updateFeatureToggles(features) {
                ...
                // FIX: Auto work button state synchronization
            ...
            updateConnectionIndicator(data) {
                ...
                const totalBots = 2 + 18;  // 2 main bots + 18 sub bots
            ...
            async initializeBots() {
                try {
                    this.logToTerminal('Đang khởi động tất cả bots...', 'info');
                    // Call backend to initialize bots
                    bot_controller.initialize_all_bots();
                    this.logToTerminal('Tất cả bots đã được khởi động', 'success');
                    setTimeout(() => this.updateStatus(), 2000);
                } catch (error) {
                    this.logToTerminal(`Lỗi khởi động bots: ${error.message}`, 'error');
                }
            }
        ...
        function toggleAutoGrab(botId) { dashboard.toggleAutoGrab(botId); }
        function setHeartThreshold(botId, threshold) { dashboard.setHeartThreshold(botId, threshold); }
        function rebootBot(botId) { dashboard.rebootBot(botId); }
        ...
        function initializeBots() { dashboard.initializeBots(); }
...
def api_status():
    ...
    return jsonify({
        ...
        'sub_bots': {
            'count': 18,  # Always show 18 acc phụ như user yêu cầu
...
def toggle_spam():
    ...
    if bot_controller.spam_enabled:
        bot_controller.spam_enabled = False
        bot_controller.spam_thread_running = False
    else:
        if bot_controller.spam_message.strip():
...
def set_spam_config():
    ...
    if message:
        bot_controller.spam_message = message
    if delay:
        bot_controller.spam_delay = delay
...
def toggle_auto_work():
    ...
    if bot_controller.auto_work_enabled:
        bot_controller.auto_work_enabled = False
        bot_controller.work_thread_running = False
    else:
...
@app.route('/api/monitor_connections', methods=['POST'])
...
def initialize_bots():
    ...
    try:
        bot_controller.initialize_all_bots()
        return jsonify({'success': True, 'message': 'All bots initialized'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
