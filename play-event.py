...
from threading import Thread, RLock # Quan trọng: Sử dụng RLock
...
def run_event_bot_thread():
    global is_bot_running, event_bot_instance
    bot = discum.Client(token=TOKEN, log=False)
    with lock:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if 200 <= r.status_code < 300:
                        ...
                        print(f"LỖI: Click thất bại! (Status: {r.status_code}, Response: {r.text}). Sẽ thử lại...", flush=True)
                        time.sleep(2)
...
def run_hourly_loop_thread():
    ...
    while is_hourly_loop_enabled:
        ...
        with lock:
            if is_hourly_loop_enabled and event_bot_instance and is_bot_running:
